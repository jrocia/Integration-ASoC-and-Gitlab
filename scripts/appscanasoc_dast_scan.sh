...
if [ -f "$loginDastConfig" ]; then
    loginDastConfigId=$(curl -s -X 'POST' "https://$serviceUrl/api/v4/FileUpload" -H 'accept:application/json' -H "Authorization:Bearer $asocToken" -H 'Content-Type:multipart/form-data' -F "uploadedFile=@$loginDastConfig" | grep -oP '(?<="FileId":\ ")[^"]*');
...
if [ -f "$manualExplorerDastConfig" ]; then
    manualExplorerDastConfigId=$(curl -s -X 'POST' "https://$serviceUrl/api/v4/FileUpload" -H 'accept:application/json' -H "Authorization:Bearer $asocToken" -H 'Content-Type:multipart/form-data' -F "uploadedFile=@$manualExplorerDastConfig" | grep -oP '(?<="FileId":\ ")[^"]*');
...
scanStatus=$(curl -s -X 'GET' "https://$serviceUrl/api/v4/Scans/Dast/$scanId" -H 'accept:application/json' -H "Authorization:Bearer $asocToken" | jq -r '.LatestExecution | {Status} | join(" ")');
echo $scanStatus
